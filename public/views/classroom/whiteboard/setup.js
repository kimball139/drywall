...
(function(credentials, mode){
	...
	var	colors = [
            {value: 'Transparent', title: 'Transparent', background: 'url(/whiteboard/icons/pr2_sprite.png) -364px -28px'},
    ...
    var canvas_backgrounds = [
        ...
        {title: "Coordinate Plane 10x10", value: "url(/whiteboard/backgrounds/coordinate-plane-10x10.png)", background: "url(/whiteboard/backgrounds-thumbs/coordinate-plane-10x10.png)"},
		{title: "Coordinate Plane Contrast 10x10", value: "url(/whiteboard/backgrounds/coordinate-plane-contrast-10x10.png)", background: "url(/whiteboard/backgrounds-thumbs/coordinate-plane-contrast-10x10.png)"},
    ...
    // synchronize tutor and student whiteboard
    ...
    var receivePeerData = function(data) {
        switch (data.type) {
            ...
            // Change role
            case 'change-role':
                // don't allow a student to change the role of the tutor
                if(user.level == 'tutor') { return;}
	...
	PR2.toolbar.addToMenu('sub_menu',  {
		...
		},
		{
			...
			access: level == "tutor" ? "always": "never",
	...
	(function() {
        ...
        chat_container.style.top = canvas.height / 2 + 30 + 'px';
	...
	PR2.peer.onConnection(function() {
		...
		// Set the user role
		setTimeout(function() { 
			PR2.user.setRole(level == 'tutor' ? 'creator': 'observer');
	...
	if (mode == 'online') {
        Socket.onUsersConnect(function() {
			PR2.modals.info('show', 'You are connected to the ' + (level == 'tutor' ? 'student': 'tutor') + '. Initializing...');	
            PR2.peer.init();
            // create peer connection if user is tutor
            if(level == 'tutor') {
                PR2.peer.connect();
            }
        });
		...
		PR2.modals.info('show', 'Waiting for ' + (level == 'tutor' ? 'student': 'tutor') + ' to join..');	
	}
